...
<details>
  <summary>대략적인 LoL 게임 설명: 롤은 5대5 팀 게임으로, 상대 진영의 포탑과 넥서스를 파괴함으로써 승리하는 게임이다. </summary>
...
## 3.3 소결
이 프로젝트의 예측 모델들은 전체 데이터와 플래티넘 이상의 경기 데이터에서 모두 높은 정확도를 보여주었다. 이는 앞서 선택한 핵심 피처들이 팀의 승패를 잘 예측하는데 효과적이라는 것을 보여주며, 각 플레이어의 개인적인 성과가 전체 팀의 승패에 큰 영향을 미친다는 LOL 게임의 특성을 잘 반영한 결과라고 할 수 있다.

또한, 모델의 성능은 플래티넘 이상의 플레이어들로 구성된 경기에서 더욱 높았다. 이는 더 높은 수준의 플레이어들이 더 일관된 성과를 내는 경향이 있기 때문일 것이다. 이 결과는 우리의 모델이 특히 높은 수준의 경기에서 더욱 정확한 예측을 할 수 있음을 보여준다. 이번 프로젝트는 이렇게 간단한 합산 모델을 사용하여도 꽤 높은 예측 정확도를 달성할 수 있음을 보여주는 좋은 예가 될 수 있다. 하지만 CNN으로 더 정확도를 높일 수 있었음도 볼 수 있었다. CNN 모델은 이미지 분석에서 많이 사용되는데, 이는 CNN이 공간적 구조 정보를 유지하면서 데이터를 처리하기 때문이다. 게임 데이터도 특성 간의 관계를 고려하면서 분석하려면 이러한 공간적 구조 정보가 유용할 수 있다. 각 게임 ID에 대해 특정 특성('kda', 'dpd', 'dpg', 'dpm', 'dtpm')의 값을 추출하고, 이를 'blue_data'와 'red_data' 리스트에 저장하는 식으로 데이터를 처리하면, 각 팀의 특성들이 CNN 모델에 입력될 때 서로 인접한 위치에 배치되었기 때문에, 모델이 특성 간의 관계를 학습하는 데 도움이 되었고 본다.

# 4. 결론 및 배운점
이 프로젝트에서 간단한 합산 모델을 사용하면서도 높은 예측 정확도를 달성하였으며, 이 결과를 통해 데이터 전처리와 피처 엔지니어링 단계에서는 각 피처가 승패에 어떠한 영향을 미치는지를 이해하는 데 큰 도움이 되었고 'kda', 'dpd', 'dpm' 등의 핵심 피처들이 승패 예측에 중요한 역할을 하는 것을 확인하였다.

또한, 모델의 성능은 플래티넘 이상의 플레이어들로 구성된 경기에서 더욱 높았다는 점은, 높은 수준의 플레이어들이 더 일관된 성과를 내는 경향이 있음을 보여주었다. 이는 모델의 성능 향상을 위해 특정 수준 이상의 플레이어들에 초점을 맞춰 데이터를 선택하는 것이 유용할 수 있음을 알려주었다. 하지만 조금더 깊은 배경 지식이 있었다면 이를 바탕으로 더 많은 피처들을 뽑아서 정확도를 더 높일 수 있지 않았을까 하는 아쉬움이 있다. 피처들을 뽑는 과정에서 많은 시행착오로 인해 시간이 소요되었지만 그럼에도 이러한 경험을 통해 데이터 분석을 하기 이전의 배경지식의 중요성을 직접 느낄 수 있어서 의미있는 경험이었다. 이 경험은 앞으로 다른 데이터 분석 프로젝트를 수행할 때도 큰 도움이 될 것이다.
