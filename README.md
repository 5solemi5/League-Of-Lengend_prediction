...
# 4. 결론 및 배운점
이 프로젝트에서 간단한 합산 모델을 사용하면서도 높은 예측 정확도를 달성하였으며, 이는 게임의 특성을 잘 이해하고 이를 반영한 결과라고 볼 수 있다.
...
또한, 모델의 성능은 플래티넘 이상의 플레이어들로 구성된 경기에서 더욱 높았다는 점은, 높은 수준의 플레이어들이 더 일관된 성과를 내는 경향이 있음을 보여주었다. 이는 모델의 성능 향상을 위해 특정 수준 이상의 플레이어들에 초점을 맞춰 데이터를 선택하는 것이 유용할 수 있음을 알려주었다. 하지만 조금더 깊은 배경 지식이 있었다면 이를 바탕으로 더 많은 피처들을 뽑아서 정확도를 높일 수 있지 않았을까 하는 아쉬움이 있다. 피처들을 뽑는 과정에서 많은 시행착오로 인해 시간이 소요되었지만 그럼에도 이러한 경험을 통해 데이터 분석을 하기 이전의 배경지식의 중요성을 직접 느낄 수 있어서 의미있는 경험이었다. 이 경험은 앞으로 다른 데이터 분석 프로젝트를 수행할 때도 큰 도움이 될 것이다.
